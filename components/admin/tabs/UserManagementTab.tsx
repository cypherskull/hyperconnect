...
const UserManagementTab: React.FC<UserManagementTabProps> = ({ allUsers, onUpdateUser }) => {
                        ...
                        {filteredUsers.map(user => (
                            <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                ...
                                <td className="px-6 py-4">
                                    <span className={`px-2 py-1 rounded text-xs font-bold ${user.persona === 'Admin' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}>
                                        {user.persona}
                                    </span>
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <button onClick={() => setEditingUser(user)} className="p-2 text-gray-500 hover:text-brand-primary rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
