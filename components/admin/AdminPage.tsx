...
import { AccessConfig, User, MonetizationRule, PlatformSettings } from '../../types';
...
const AdminPage: React.FC<AdminPageProps> = (props) => {
    const { onNavigateBack, allUsers, accessConfig, onUpdateAccessConfig, monetizationRules, onAddRule, onUpdateRule, onDeleteRule, onUpdateUser } = props;
    const [activeTab, setActiveTab] = useState<AdminTab>('users');
    const { state, handleUpdatePlatformSettings } = useAppContext();

    const renderContent = () => {
        switch (activeTab) {
            case 'users':
                return <UserManagementTab allUsers={allUsers} onUpdateUser={onUpdateUser} />;
            case 'access':
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <aside className="md:w-1/4 lg:w-1/5">
                        <nav className="flex flex-col space-y-2">
                           <AdminTabButton icon={<WrenchScrewdriverIcon className="w-5 h-5"/>} label="Platform Settings" isActive={activeTab === 'settings'} onClick={() => setActiveTab('settings')} />
