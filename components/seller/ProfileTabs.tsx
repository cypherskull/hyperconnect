...
const EventsTab: React.FC<{ events: Event[], onMessageProgramManager?: (managerId: string, eventTitle: string) => void }> = ({ events, onMessageProgramManager }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {events.map(e => (
            <EventCard key={e.id} event={e} onMessageProgramManager={onMessageProgramManager} />
        ))}
    </div>
);
    ...
    <div>
        ...
        <div className="space-y-4">
            ...
            {(!contacts || contacts.length === 0) && (
                <p className="text-gray-500 italic">No key contacts available.</p>
            )}
        </div>
    </div>
);
...
export const ProfileTabs: React.FC<ProfileTabsProps> = ({ seller, solution, posts, isOwner, isConnection, onMessageProgramManager }) => {
    ...
    const availableTabs = useMemo(() => {
        ...
        const visibleEvents = activeSolution.events?.filter(e => e.isPublic || isConnection || isOwner) || [];
        if (visibleEvents.length > 0) tabs.push({ key: 'events', label: 'Events', count: visibleEvents.length });
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'events': return <EventsTab events={activeSolution.events?.filter(e => e.isPublic || isConnection || isOwner) || []} onMessageProgramManager={onMessageProgramManager} />;
