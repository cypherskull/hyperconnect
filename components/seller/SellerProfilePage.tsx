...
import { Seller, Post as PostType, User, Solution, InboxItem, DueDiligenceData, ProfileTab } from '../../types';
import { ProfileTabs } from './ProfileTabs';
import { PostsTab } from './PostsTab';
import { ManageTab } from './ManageTab';
import { SellerDashboardPage } from './dashboard/SellerDashboardPage';
import { ArrowLeftIcon, UsersIcon, CurrencyDollarIcon, CheckCircleIcon } from '../common/Icons';
...
interface SellerProfilePageProps {
    ...
    onMessageProgramManager?: (managerId: string, title: string) => void;
}
...
const SellerHeader: React.FC<{
    ...
    const ConnectionButton = () => {
        switch (connectionStatus) {
            case 'connected':
                return (
                    <button disabled className="w-full sm:w-auto px-4 py-2 rounded-md font-semibold text-sm bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 cursor-default flex items-center justify-center gap-2">
            ...
            case 'pending':
                return (
                    <button disabled className="w-full sm:w-auto px-4 py-2 rounded-md font-semibold text-sm bg-brand-primary/10 text-brand-primary dark:bg-brand-primary/20 cursor-default flex items-center justify-center gap-2">
                        <CheckCircleIcon className="w-4 h-4" />
                        Request Sent
                    </button>
                );
            default:
                return <button id="btn-connect" onClick={onSendConnectionRequest} className="w-full sm:w-auto px-4 py-2 rounded-md font-semibold text-sm bg-brand-primary text-white hover:bg-brand-secondary">Connect</button>;
        }
    }

    return (
        <div id="seller-profile-page" className="bg-brand-card rounded-lg shadow-md p-6 border border-brand-border">
            <div className="flex flex-col sm:flex-row items-start gap-6">
                ...
                <div className="flex-grow">
                    ...
                    <div className="flex items-center gap-4 text-gray-500 dark:text-gray-400 mt-1">
                        <span>{(seller.solutions?.[0]?.industries ?? seller.solutions?.[0]?.industry ?? []).join(', ')}</span>
                        <div className="flex items-center gap-1">
                            <UsersIcon className="w-4 h-4" />
                            <span className="font-semibold">{(seller.followers ?? 0).toLocaleString()}</span>
                            <span>followers</span>
                        </div>
                    </div>
                ...
                {!isOwner && (
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 flex-shrink-0 w-full sm:w-auto">
                        <ConnectionButton />
                        <button id="btn-message" onClick={() => { console.log("CLICKING MESSAGE"); onMessage(); }} className="w-full sm:w-auto px-4 py-2 rounded-md font-semibold text-sm bg-brand-secondary text-white">Message</button>
                        <button id="btn-meeting" onClick={onMeeting} className="w-full sm:w-auto px-4 py-2 rounded-md font-semibold text-sm border border-brand-border">Request Meeting</button>
                    </div>
                )}
            </div>
            {!isOwner && (
                <div className="mt-4 pt-4 border-t border-brand-border flex items-center gap-4">
                    <button id="btn-follow" onClick={onFollowSeller} className={`font-semibold text-sm ${isFollowed ? 'text-brand-primary' : 'text-gray-500'}`}>
                        {isFollowed ? 'Following' : 'Follow'}
                    </button>
                    <button id="btn-testimonial" onClick={onAddTestimonial} className="font-semibold text-sm text-gray-500 hover:text-brand-primary">
                        Write a Testimonial
                    </button>
                </div>
            )}
...
export const SellerProfilePage: React.FC<SellerProfilePageProps> = (props) => {
    const { seller, allPosts, allSellers, currentUser, inboxItems, onBack, onSelectSeller, onUpdateSolutions, onOpenPostCreator, onAddTestimonial, onSendConnectionRequest, onMessage, onMeeting, initialPostId, onActivateSolution, onSetSolutionHold, onCancelSolutionHold, onToggleInvestmentStatus, onUpdateDueDiligence, onMessageProgramManager } = props;
        ...
        if (props.initialTab === 'posts' && initialPostId) {
            ...
            setTimeout(() => {
                ...
                if (postElement) {
                    ...
                    el.style.backgroundColor = 'var(--color-primary-accent-translucent)'; // A temporary highlight color
                    setTimeout(() => {
    ...
    // Show all posts for this seller (backend solutions have no `status` field)
    const sellerPosts = useMemo(() => allPosts.filter(p => p.sellerId === seller.id), [allPosts, seller.id]);
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'analytics':
                return isOwner ? <SellerDashboardPage seller={seller} isEmbedded allSellers={allSellers} /> : <p>Access denied.</p>;
            case 'investment':
            ...
            case 'about':
            default:
                return <ProfileTabs seller={seller} solution={selectedSolution} posts={sellerPosts} isOwner={isOwner} isConnection={connectionStatus === 'connected'} onMessageProgramManager={onMessageProgramManager} />;
        }
    };
