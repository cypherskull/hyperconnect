            ...
            <div className="space-y-2">
                {(solution.caseStudies || []).map(cs => (
                    <div key={cs.id} className="p-3 bg-brand-card-light rounded-md flex justify-between items-center">
                        ...
                        {confirmingDelete === cs.id && (
                            <div className="absolute inset-x-0 bottom-0 top-0 bg-brand-card/95 z-10 flex items-center justify-between px-4 rounded-md border-2 border-red-500/50">
                                <span className="text-sm font-bold text-red-600">Delete this case study?</span>
                                <div className="flex gap-2">
                                    <button onClick={() => confirmDelete(cs.id)} className="px-3 py-1 bg-red-600 text-white text-xs font-bold rounded hover:bg-red-700">Confirm</button>
                                    <button onClick={() => setConfirmingDelete(null)} className="px-3 py-1 bg-gray-200 dark:bg-gray-700 text-xs font-bold rounded">Cancel</button>
                                </div>
                            </div>
                        )}
                    </div>
...
const PodcastsManager: React.FC<{ podcasts: Podcast[]; onUpdate: (data: Partial<Solution>) => void }> = ({ podcasts, onUpdate }) => {
    ...
    return (
        <div>
            <Button onClick={() => showToast('info', 'Coming Soon', 'Podcast management coming soon!')} className="w-full">
                <UploadIcon className="w-5 h-5" />
...
// --- EVENTS TAB --- //
const EventsManager: React.FC<{ solution: Solution; onUpdate: (data: Solution) => void }> = ({ solution, onUpdate }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    ...
    const handleSave = (eventData: Event) => {
        ...
        onUpdate({ ...solution, events: updatedEvents });
        setIsModalOpen(false);
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                {(!solution.events || solution.events.length === 0) && (
                    <p className="text-center text-sm text-gray-500 mt-4">No events found. Start by hosting a new event.</p>
                )}
            </div>
...
export const SellerContentManager: React.FC<SellerContentManagerProps> = ({ solution, onUpdate, onShare }) => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'events':
                return <EventsManager solution={solution} onUpdate={onUpdate} />;
        }
    }
