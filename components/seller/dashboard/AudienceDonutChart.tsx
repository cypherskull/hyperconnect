    ...
    return (
        <div className="bg-brand-card rounded-lg shadow-md p-6 border border-brand-border h-full">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="w-full flex-1 space-y-2 text-sm min-w-0">
                    {chartData.map((item, index) => (
                        <div key={index} className="flex justify-between items-center gap-2">
                            <div className="flex items-center min-w-0">
                                <span className="w-3 h-3 flex-shrink-0 rounded-full mr-2" style={{ backgroundColor: item.color }}></span>
                                <span className="truncate">{item.name}</span>
                            </div>
                            <span className="font-semibold whitespace-nowrap flex-shrink-0">{item.value} ({item.percentage.toFixed(0)}%)</span>
                        </div>
                    ))}
                </div>
            </div>
