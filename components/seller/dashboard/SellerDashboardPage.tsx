...
const SubTabButton: React.FC<{ active: boolean; onClick: () => void; children: React.ReactNode }> = ({ active, onClick, children }) => (
  <button
    onClick={onClick}
    className={`px-4 py-2 font-semibold border-b-2 ${active
...
export const SellerDashboardPage: React.FC<SellerDashboardPageProps> = ({ seller, isEmbedded = false, allSellers }) => {
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="pt-4">
        {activeSubTab === 'performance' ? (
          ...
          <GlobalInsightsTab seller={seller} allSellers={allSellers} />
        )}
      </div>
    </div>
  );
