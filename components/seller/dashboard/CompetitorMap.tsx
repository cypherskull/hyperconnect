...
const ToggleButton: React.FC<{ active: boolean; onClick: () => void; children: React.ReactNode }> = ({ active, onClick, children }) => (
    <button
        onClick={onClick}
        className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-colors ${active ? 'bg-brand-secondary text-white' : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600'
            }`}
    >
        {children}
    </button>
...
const CompetitorChart: React.FC<CompetitorChartProps> = ({ currentSeller, allSellers }) => {
    ...
    const chartData: BarChartData[] = useMemo(() => {
        ...
        // Use real sellers if provided; fall back to mock data
        const sellersPool = allSellers && allSellers.length > 0 ? allSellers : mockSellers;
        if (!primaryIndustry) return [];

        const competitors = sellersPool.filter(s =>
