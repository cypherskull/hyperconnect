...
interface PostEditorModalProps {
  ...
  onClose: () => void;
  onSave: (postData: Omit<Post, 'id' | 'sellerId' | 'likes' | 'bookmarks' | 'comments' | 'isLiked' | 'isBookmarked' | 'timestamp'> & { id?: string }) => void;
}
...
export const PostEditorModal: React.FC<PostEditorModalProps> = ({ solution, shareItem, postToEdit, onClose, onSave }) => {
  ...
  const handleAddMedia = (type: Media['type']) => {
      if (media.length >= 4) {
          showToast('warning', 'Media Limit Reached', 'You can add a maximum of 4 media items.');
          return;
      }
      if (type === 'audio') {
          showToast('info', 'Not Available', 'Audio uploads are not implemented in this demo.');
          return;
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title) {
        setTitleError('Please provide a title for your post.');
        return;
    }
    setTitleError(null);
      ...
      <div className="bg-brand-card rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-4 flex-grow">
          <div>
            ...
            <Input value={title} onChange={e => { setTitle(e.target.value); setTitleError(null); }} required placeholder="An engaging title for your post" />
