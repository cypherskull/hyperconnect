...
import { User, Enterprise, Persona, Seller, Transaction, PaymentMethod, ProfileTab } from '../../types';
...
export const ProfileManagementPage: React.FC<ProfileManagementPageProps> = (props) => {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="bg-brand-card rounded-lg p-6 shadow-md border border-brand-border flex flex-col sm:flex-row items-center sm:items-start gap-6 relative">
                <img src={user.avatarUrl} alt={user.name} className="w-24 h-24 rounded-full border-4 border-brand-card-light" />
                <div className="text-center sm:text-left flex-grow">
                    <h1 className="text-3xl font-bold">{user.name}</h1>
                    <p className="text-gray-500 dark:text-gray-400 text-lg">{user.designation} at <span className="font-semibold text-brand-primary">{user.company}</span></p>
                    <p className="text-sm text-gray-400 mt-1">{user.persona} Account</p>
                </div>
                <button
                    onClick={() => setIsEditProfileModalOpen(true)}
                    className="absolute top-4 right-4 p-2 text-gray-400 hover:text-brand-primary hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors"
                    title="Edit Profile"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <SectionCard title="Personal Information">
                    ...
                    <ProfileField label="Phone Number" value={user.phone} onEdit={() => setIsEditProfileModalOpen(true)} />
                </SectionCard>
                ...
                <SectionCard title="Interests & Subscription">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => showToast('info', 'Coming Soon', 'Plan upgrades coming soon!')} className="text-sm text-brand-secondary hover:underline mt-2">Upgrade Plan</button>
                    </div>
                </SectionCard>
            </div>
