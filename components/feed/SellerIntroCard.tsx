...
import { LikeIcon, CommentIcon, PencilIcon, QuoteIcon, FileTextIcon, PresentationIcon, ImageIcon, VideoCameraIcon, BookmarkIcon, StarIcon } from '../common/Icons';
...
const ContentPreview: React.FC<{ post: PostType }> = ({ post }) => {
    ...
    const backgroundImageUrl = media?.find(m => m.type === 'image')?.url || `https://picsum.photos/seed/${post.id}/400/600`;
                ...
                <div className="p-4 h-full w-full flex flex-col justify-center items-center text-center text-white bg-blue-900/30">
                    ...
                    <div className="mt-4 flex flex-col gap-2">
                        <div className="flex items-center justify-center space-x-2">
                            <span className="text-xs opacity-80 w-24 text-right">Impact:</span>
                            <div className="flex">
                                {[...Array(5)].map((_, i) => (
                                    <StarIcon key={i} className={`w-3 h-3 ${i < testimonialItem.scores.solutionImpact ? 'text-yellow-400' : 'text-gray-400/50'}`} />
                                ))}
                            </div>
                        </div>
                        <div className="flex items-center justify-center space-x-2">
                            <span className="text-xs opacity-80 w-24 text-right">Experience:</span>
                            <div className="flex">
                                {[...Array(5)].map((_, i) => (
                                    <StarIcon key={i} className={`w-3 h-3 ${i < testimonialItem.scores.experience ? 'text-yellow-400' : 'text-gray-400/50'}`} />
                                ))}
                            </div>
                        </div>
                    </div>
