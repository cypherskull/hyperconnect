...
const ActionButton: React.FC<{ onClick: (e: React.MouseEvent) => void, children: React.ReactNode, 'aria-label': string, isActive?: boolean, disabled?: boolean, title?: string }> =
    ({ onClick, children, 'aria-label': ariaLabel, isActive = false, disabled = false, title }) => (
        <button
            ...
            disabled={disabled}
            title={title}
            className={`flex-1 flex flex-col items-center justify-center p-2 rounded-md transition-colors ${isActive
...
export const SellerCard: React.FC<SellerCardProps> = React.memo((props) => {
    ...
    const connectionStatus = useMemo(() => {
        ...
        const pendingRequest = inboxItems.find(item =>
            ...
            (item.relatedSellerId === seller.id || item.relatedUserId === seller.id)
        );
    ...
    const ConnectionContent = () => {
        switch (connectionStatus) {
            ...
            case 'pending': return <><CheckCircleIcon className="w-5 h-5 text-brand-primary" /><span className="text-[10px] mt-1 font-semibold text-brand-primary">Sent</span></>;
        ...
        >
            <div className="p-4 flex-grow cursor-pointer" onClick={() => onSelectSeller(seller)}>
                ...
                <div className="flex justify-around items-center pt-3 border-t border-brand-secondary/20 dark:border-brand-border">
                    <Stat label="Clients" value={seller.businessStats?.clients ?? seller.clients ?? 0} />
                    <Stat label="Followers" value={(seller.followers ?? 0).toLocaleString()} />
                    <Stat label="Score" value={seller.platformScore ?? 0} />
                </div>
            </div>
