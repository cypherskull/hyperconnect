...
const categories: { name: TabCategory, icon: React.FC<any> }[] = [
    ...
    { name: 'ConnectionRequest', icon: UsersIcon },
    { name: 'MeetingRequest', icon: CalendarIcon },
    { name: 'SalesEnquiry', icon: CurrencyDollarIcon },
    { name: 'Engagement', icon: LikeIcon },
    { name: 'CollaborationRequest', icon: UsersIcon },
...
export const InboxPage: React.FC<InboxPageProps> = ({ allInboxItems, currentUser, onRespondToConnectionRequest, onRespondToItem }) => {
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <h1 id="inbox-heading" className="text-3xl font-bold mb-6">Inbox</h1>
            <div className="flex border-b border-brand-border mb-6 overflow-x-auto scrollbar-hide">
                {categories.map(({ name, icon: Icon }) => {
                    ...
                    return (
                        <button id={`inbox-tab-${name.toLowerCase().replace(/\s+/g, '-')}`} key={name} onClick={() => setActiveTab(name)} className={`flex items-center flex-shrink-0 space-x-2 px-4 py-3 font-semibold border-b-2 text-sm ${activeTab === name ? 'border-brand-primary text-brand-primary' : 'border-transparent text-gray-500 hover:text-brand-primary'}`}>
