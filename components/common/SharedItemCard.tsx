...
const TestimonialCard: React.FC<{ item: Testimonial }> = ({ item }) => (
    <div className="bg-brand-card-light/50 dark:bg-brand-card-light/10 rounded-lg p-4 border border-brand-border/50">
        ...
        <div className="mt-3 flex flex-col space-y-1">
            <div className="flex items-center justify-end space-x-1 text-yellow-400">
                <span className="text-xs text-gray-500 dark:text-gray-400">Solution Impact:</span>
                {[...Array(5)].map((_, i) => (
                    <StarIcon key={`impact-${i}`} className={`w-3 h-3 ${i < item.scores.solutionImpact ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`} />
                ))}
            </div>
            <div className="flex items-center justify-end space-x-1 text-yellow-400">
                <span className="text-xs text-gray-500 dark:text-gray-400">Experience:</span>
                {[...Array(5)].map((_, i) => (
                    <StarIcon key={`exp-${i}`} className={`w-3 h-3 ${i < item.scores.experience ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`} />
                ))}
            </div>
        </div>
    </div>
