...
export const MeetingInviteModal: React.FC<MeetingInviteModalProps> = ({ mode, onClose, onSave }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!meetingLink.trim() || !meetingLink.startsWith('http')) {
        setError('Please enter a valid meeting URL starting with http.');
        return;
    };
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-brand-card rounded-lg shadow-xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {error && (
            <div className="mx-6 mb-2 px-3 py-2 rounded-md bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm">
              {error}
            </div>
          )}
